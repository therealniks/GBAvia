...
- (instancetype)init {
    self = [super init];
    if (self) {
        _imageView = [[UIImageView alloc] initWithFrame:CGRectMake([UIScreen
        mainScreen].bounds.size.width/2 - 100.0, [UIScreen mainScreen].bounds.size.height/2 - 100.0, 200.0,
        200.0)];
        ...
        _contentLabel = [[UILabel alloc] initWithFrame:CGRectMake([UIScreen
        mainScreen].bounds.size.width/2 - 100.0, CGRectGetMaxY(_imageView.frame) + 20.0, 200.0, 21.0)];
...
- (void)setTitle:(NSString *)title {
    ...
    CGRectGetMinY(_imageView.frame) - 40.0 - height, 200.0, height);
...
- (void)setContentText:(NSString *)contentText {
    ...
    CGRectGetMaxY(_imageView.frame) + 20.0, 200.0, height);
}
