...
@interface PlaceViewController ()
    ...
    @property (nonatomic, strong) NSArray *currentArrayS;
...
-(void) changeSource {
    switch (_segmentedControl.selectedSegmentIndex) {
        case 0:
            _currentArrayS = [[DataManager sharedInstance] cities];
            break;
        case 1:
            _currentArrayS = [[DataManager sharedInstance] airports];
            break;        default:
            break;
    }
...
- (nonnull UITableViewCell *)tableView:(nonnull UITableView *)tableView cellForRowAtIndexPath:(nonnull NSIndexPath *)indexPath {
    ...
    if (_segmentedControl.selectedSegmentIndex == 0) {
        City *city = [_currentArrayS objectAtIndex:indexPath.row];
        cell.textLabel.text = city.name;
        cell.detailTextLabel.text = city.code;
    }
    else if (_segmentedControl.selectedSegmentIndex == 1) {
        Airport *airport = [_currentArrayS objectAtIndex:indexPath.row];
...
- (NSInteger)tableView:(nonnull UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return [_currentArrayS count];
}

- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    
    DataSourceType dataType = ((int) _segmentedControl.selectedSegmentIndex) + 1;
    [self.delegate selectPlace:[_currentArrayS objectAtIndex:indexPath.row] withType:_placeType andDataType:dataType];
